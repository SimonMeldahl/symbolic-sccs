...
import symbolic.*
import symbolic.BDD.*
import symbolic.Main.{e, gen}

object Test {
  val INF = 100000000

  def testAll(sccAlg: (BDD, BDD, BDD) => List[BDD]): Unit = {
    val r = scala.util.Random
    for (c <- 1 to 25) for (n <- 1 to 25) for (m <- 1 to 25)
